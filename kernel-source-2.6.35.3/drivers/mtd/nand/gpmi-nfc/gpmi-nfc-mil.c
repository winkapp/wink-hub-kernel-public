...
static int mil_boot_areas_init(struct gpmi_nfc_data *this)
{
	...
	unsigned int                   i;
	struct mtd_partition           partitions[6]; //ssen - changed this to 6 from 4 to accomodate all partitions.
	...
	if (rom->boot_area_count == 1) {
		 ...
		 * +------+----------------------------------------------------+
		 */
#if 0 //ssen - updating the NAND flash map for accomodating more partitions
		/* Chip 0 Boot */
		...
		partitions[0].name         = chip_0_boot_name;
		partitions[0].offset         = 0;
		partitions[0].size            = (3 * 1024 * 1024);
		partitions[0].mask_flags = 0;

		/* Chip 0 Updater-kernel */

		partitions[1].name         = "updater-kernel";
		partitions[1].offset         = (3 * 1024 * 1024);
		partitions[1].size            = (4 * 1024 * 1024);
		partitions[1].mask_flags = 0;

		/* Chip 0 Updater-rootfs */

		partitions[2].name         = "updater-rootfs";
		partitions[2].offset         = (7 * 1024 * 1024);
		partitions[2].size            = (28 * 1024 * 1024);
		partitions[2].mask_flags = 0;

		/* Chip 0 Database */

		partitions[3].name         = "database";
		partitions[3].offset         = (35 * 1024 * 1024);
		partitions[3].size            = (8 * 1024 * 1024);
		partitions[3].mask_flags = 0;

		/* Chip 0 Application Kernel */

		partitions[4].name         = "app-kernel";
		partitions[4].offset         = (43 * 1024 * 1024);
		partitions[4].size            = (8 * 1024 * 1024);
		partitions[4].mask_flags = 0;

		/* Chip 0 Application-rootfs (General Use) */

		partitions[5].name       = general_use_name;
		partitions[5].offset     = (51 * 1024 * 1024);
		partitions[5].size       = MTDPART_SIZ_FULL;
		partitions[5].mask_flags = 0;

		/* Construct and register the partitions. */

		add_mtd_partitions(mtd, partitions, 6);
#endif
